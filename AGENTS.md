...
([.agents/orchestrator/orchestrator.py](.agents/orchestrator/orchestrator.py)). It is a pure **instruction printer**: it never
...
```
.agents/guidelines/       engineering best practices referenced by the subagents: core.md (shared:
                            KISS/DRY/YAGNI/SOLID/…) + per-domain requirements/design/testing/
                            implementation/review files

.agents/state/<task_id>/  per-task persisted state: append-only state.json history plus the
...
`orchestrator.py` commands default to persisting state under `.agents/state` at the repo root, regardless
of the caller's cwd:

```bash
python3 .agents/orchestrator/orchestrator.py add --task <id> <<< "requirements text"
python3 .agents/orchestrator/orchestrator.py next --task <id>       # read-only: what to run next
python3 .agents/orchestrator/orchestrator.py record --task <id>     # record verdict.json, advance, print next
python3 .agents/orchestrator/orchestrator.py confirm --task <id> --approve
python3 .agents/orchestrator/orchestrator.py status --task <id>
```
...
zero API calls and no subagents via `run --stub-scenario .agents/orchestrator/test/scenarios/<name>.json`
